...
1. Start with EnthusiaSignature alone. As a non-op, use `/sign`, read the permanent warning/preview, then `/sign confirm`. Confirm the item stays unchanged before confirmation, then gains the name and date. Retry signing and verify rejection. Verify EnthusiaSignature registers no `/lore` command.
...
10. Change settings and use `/enthusiasignature reload` (and verify the legacy `/itemsignature reload` still works). Invalid date/timezone/limits or malformed YAML must reject the reload while previous settings keep working. Existing signatures retain their original rendering/date; future signatures and tracker redraws use new formats.
...
13. On a copy of a server with `plugins/ItemSignature/config.yml`, remove the old JAR and start only EnthusiaSignature. Confirm the old config is unchanged, `plugins/EnthusiaSignature/config.yml` contains the same custom values, both admin command names work, existing signed/tracked items retain their `itemsignature:*` data, and LuckPerms grants still apply. Repeat with a custom new config already present; it must not be overwritten. Never test first-run migration against the sole production copy.
