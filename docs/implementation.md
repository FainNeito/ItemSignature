...
## Infrastructure
ItemSignaturePlugin is the composition root, command adapter and reload boundary.
...
No git repository exists in this workspace, so upstream init/refine commit steps are inapplicable; changes are delivered as files and a JAR.
